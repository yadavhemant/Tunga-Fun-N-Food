...
<html>
    ...
    <%
          ...
          try{
            if((name=="" || add=="" || contact=="" || qty=="" || date==""))
                {
                 out.println("<script>alert('All fields must be complete.')</script>");
                      response.sendRedirect("rbooking.jsp");
                }
                else
                    {
           ...
           s=con.prepareStatement("select count(*) from rbooking");
           rs=s.executeQuery();
           rs.next();
           int orno=rs.getInt(1)+1;
           s1=con.prepareStatement("select * from rdetails where rcode='"+rcode+"'");
               rs1=s1.executeQuery();
                 if(rs1.next())
                    {
                    String rn=rs1.getString(2);
                    String rc=rs1.getString(3);
                    int price=rs1.getInt(7);
                    double total=price * Integer.parseInt(qty);
                    s2=con.prepareStatement("insert into rbooking values("+orno+",'"+name+"','"+add+"',"+contact+",'"+rcode+"','"+rn+"','"+rc+"',"+qty+",'"+date+"',"+total+")");
                    s2.executeUpdate();
